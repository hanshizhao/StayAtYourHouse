...
test.describe('FEAT-099: 后端 API 层 — 新增确认接口端点', () => {
  ...
  test('RentalAppService 包含 ConfirmAnJuCode 方法', () => {
    ...
    test.expect(content).toContain('public async Task<RentalRecordDto> ConfirmAnJuCode(int id, string? anJuCodeRegisteredNames = null)');
  });
  ...
  test('ConfirmAnJuCode 方法调用 _rentalRecordService.ConfirmAnJuCodeAsync', () => {
    ...
    test.expect(methodBody).toContain('_rentalRecordService.ConfirmAnJuCodeAsync(id, anJuCodeRegisteredNames)');
  });
