...
test.describe("FEAT-028: 利润排行页", () => {
  ...
  test("12. 排序功能 - 可以按列排序", async ({ page }) => {
    ...
    if (await table.isVisible()) {
      // 查找可排序的表头
      const sortableHeaders = table.locator(
        'th[class*="sort"], .t-table__th--sortable',
      );
      ...
      if (count > 0) {
        ...
        // 验证排序状态变化
        const sortIcon = table.locator(
          '[class*="sort-icon"], .t-table__sort-icon',
        );
  ...
  test("15. 分页大小 - 可以修改每页条数", async ({ page }) => {
    ...
    if (await pagination.isVisible()) {
      // 查找每页条数选择器
      const pageSizeSelect = pagination.locator(
        '[class*="select"], .t-pagination__select',
      );
