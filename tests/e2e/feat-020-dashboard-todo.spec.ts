...
test.describe('FEAT-020: 首页待办提醒', () => {
  ...
  test('4. 待办项数据结构 - 租客信息', async ({ page }) => {
    await loginAndNavigateToDashboard(page);
  ...
  test('5. 待办项数据结构 - 房间信息', async ({ page }) => {
    await loginAndNavigateToDashboard(page);
  ...
  test('6. 待办项数据结构 - 费用金额', async ({ page }) => {
    await loginAndNavigateToDashboard(page);
    ...
    if (await todoItem.count() > 0) {
      const text = await todoItem.textContent();
      // 应该包含金额信息
      expect(text).toMatch(/\d+.*元|费用|金额/);
    }
  });

  test('7. 待办项 - 操作按钮存在', async ({ page }) => {
    await loginAndNavigateToDashboard(page);

    // 找到任意待办项的操作按钮（缴费等）
    const actionButton = page.locator('[data-testid="todo-action-button"], button:has-text("缴费"), button:has-text("收款")');
    if (await actionButton.count() > 0) {
      await expect(actionButton.first()).toBeVisible();
    }
  });

  test('8. 水电费待办 - 待缴费状态', async ({ page }) => {
    await loginAndNavigateToDashboard(page);

    // 查找待缴费的水电费待办项
    const pendingUtilityBill = page.locator('[data-testid="pending-utility-bill"], .todo-item:has-text("待缴费"), .todo-item:has-text("未付")');
    if (await pendingUtilityBill.count() > 0) {
      await expect(pendingUtilityBill.first()).toBeVisible();
    }
  });

  test('9. 空状态处理 - 无待办时显示提示', async ({ page }) => {
    await loginAndNavigateToDashboard(page);
  ...
  test('12. 待办项点击 - 跳转到详情', async ({ page }) => {
    await loginAndNavigateToDashboard(page);
    ...
    if (await todoItem.count() > 0) {
      ...
      // 验证跳转（可能跳转到水电费页或租客详情页）
      const url = page.url();
      expect(url.includes('dashboard') || url.includes('utility') || url.includes('rental')).toBeTruthy();
    }
  });

  test('13. 待办分类图标', async ({ page }) => {
    await loginAndNavigateToDashboard(page);
  ...
  test('16. 快捷操作入口', async ({ page }) => {
    await loginAndNavigateToDashboard(page);

    // 查找快捷操作入口（如：快速缴费、快速入住等）
