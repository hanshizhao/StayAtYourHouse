...
export async function loginAndNavigate(
  page: Page,
  targetPath: string,
  credentials: { username: string; password: string } = { username: 'admin', password: 'admin123' }
): Promise<void> {
  await page.goto(`${BASE_URL}/auth/sign-in`);

  // 等待登录表单加载
  await page.waitForSelector('[data-testid="username-input"], input[placeholder*="用户名"]', {
    timeout: 10000
  });
  ...
  const submitButton = page.locator('[data-testid="login-button"]').first()
...
export function getCriticalErrors(errors: string[]): string[] {
  ...
  return errors.filter(e =>
    !ignorePatterns.some(pattern => e.includes(pattern))
  );
}
...
export async function getCellText(
  ...
  const cell = page.locator(`${tableSelector} tbody tr`).nth(rowIndex).locator('td').nth(columnIndex);
 ...
 * 等待成功 Toast
 */
export async function waitForSuccessToast(page: Page, timeout = 10000): Promise<string> {
 ...
 * 等待错误 Toast
 */
export async function waitForErrorToast(page: Page, timeout = 10000): Promise<string> {
  const toastSelectors = [
 ...
 * 断言 API 响应成功
 */
export async function assertApiSuccess(response: {
  status: () => number;
  json: () => Promise<any>;
}, expectedStatus = 200): Promise<any> {
 ...
 * 断言 API 响应失败
 */
export async function assertApiFailure(response: {
  status: () => number;
  json: () => Promise<any>;
}, expectedStatus = 400): Promise<any> {
  expect(response.status()).toBe(expectedStatus);
  const result = await response.json();
