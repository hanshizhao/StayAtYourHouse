...
按规格"暂时禁用"的决策，将方法简化为返回空数据（租金收入归零，后续按需重新设计）：
- 删除第 43-47 行（查询 paidBills）
- 将第 86-88 行的 `rentIncome` 改为 `0`：
```csharp
            var rentIncome = 0m;
```
...
- [ ] **Step 3: 修改 UtilityBillDto.cs — 清理 Merged 并新增 RentalRecordId**

删除 `StatusText` 中的 Merged 分支（第 88-94 行改为）：
```csharp
...
新增 `RentalRecordId` 属性（在 `RoomId` 属性之后）：
```csharp
    /// <summary>
    /// 租住记录ID
    /// </summary>
...
同时在类声明上方的 `[Index]` 属性区域（第 55-56 行）添加新索引：
```csharp
...
- [ ] **Step 1: 清理 Bill 相关导入和函数**

- 删除 `import { BillStatus } from '@/api/model/billModel';`（如存在）
- 新增 `import { UtilityBillStatus, UtilityBillStatusText } from '@/api/model/meterModel';`
- 删除或替换 `getBillStatusTheme` 函数为 `getUtilityBillStatusTheme`：

```typescript
function getUtilityBillStatusTheme(status: UtilityBillStatus): 'warning' | 'success' {
  const themes: Record<UtilityBillStatus, 'warning' | 'success'> = {
    [UtilityBillStatus.Pending]: 'warning',
    [UtilityBillStatus.Paid]: 'success',
  };
...
- 将 `暂无关联账单` 改为 `暂无水电费账单`
- 将 `关联账单` 改为 `水电费账单`
- 状态标签使用 `getUtilityBillStatusTheme` 和 `UtilityBillStatusText`

列定义替换为水电费账单列（删除原来的 `billColumns`）：
```typescript
...
- 删除 `UtilityBillStatus.Merged = 2` 枚举值和对应的 `UtilityBillStatusText` 中 Merged 的文本映射
- 在 `UtilityBillItem` 接口中新增 `rentalRecordId?: number` 字段

- [ ] **Step 2: 修改 UtilityBillDto Mapper 映射**

在 `Mapper.cs` 中更新 UtilityBill → UtilityBillDto 映射，新增 `RentalRecordId` 映射：

```csharp
        config.NewConfig<UtilityBill, UtilityBillDto>()
            .Map(dest => dest.TenantId, src => src.BillTenantId)
            .Map(dest => dest.RentalRecordId, src => src.RentalRecordId);
```
