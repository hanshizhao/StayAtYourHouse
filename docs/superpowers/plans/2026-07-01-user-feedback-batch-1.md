...
### Task A2: 第 10 条 — 编辑报修房间显示数字"1"（前端）
...
### Task A4: 第 3 条 — 待办面板显示合同到期日与逾期天数（前端）
...
### Task B1: 第 5 条 — 已退租房间转「回收」状态（后端+前端）
...
### Task B3: 第 4 条 — 安居码登记人员字段（后端含迁移 + 前端）
...
### Task B4: 第 7 条 — 抄表加入固定费用（后端含迁移 + 前端）
...
### Task C1: 第 9 条 — 合同图片上传（后端接口 + 前端 token）
