...
#### 宽限 API 验证
- `deferredToDate` 必须 > 今天
- `deferredToDate` - 原提醒日期 ≤ 30 天（宽限上限）
- 后端返回验证错误信息

#### 续租 API 验证
- `monthlyRent` 必须 > 0
- `contractEndDate` 必须 > 原合同结束日期
- `contractEndDate` 必须 > 今天

### 手机号脱敏处理
- **后端处理**：在 DTO 中返回脱敏后的手机号（如 `138****1234`）
- 前端直接显示，无需额外处理
...
### 宽限次数显示
- 当 `deferralCount > 0` 时显示"已宽限 [N] 次"链接
- 当 `deferralCount = 0` 时隐藏该行

### 待办列表分页
- 支持 `page` 参数（默认1）和 `pageSize` 参数（默认10）
- 落地页场景通常只显示前10条，分页主要用于未来扩展
